...
interface Product { id: string; name: string; category_id: string; price: number; description: string; image_url: string; created_at: string; categories?: { name: string } | null; }

type SortOption = 'newest' | 'price_low' | 'price_high' | 'name_asc';
const sortLabels: Record<SortOption, string> = { newest: 'Newest First', price_low: 'Price: Low → High', price_high: 'Price: High → Low', name_asc: 'Name: A–Z' };
...
const ProductsManager = () => {
  ...
  const resetForm = () => { setName(''); setCategoryId(''); setPrice(''); setDescription(''); setImageFile(null); setEditingProduct(null); };

  const openEdit = (product: Product) => {
    setEditingProduct(product); setName(product.name); setCategoryId(product.category_id);
    setPrice(String(product.price)); setDescription(product.description); setDialogOpen(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !categoryId || !price || !description.trim()) { toast({ title: 'Error', description: 'Please fill all required fields', variant: 'destructive' }); return; }
    ...
    try {
      ...
      if (editingProduct) {
        const updateData: any = { name: name.trim(), category_id: categoryId, price: priceNum, description: description.trim() };
        if (imageUrl) updateData.image_url = imageUrl;
        const { error } = await supabase.from('products').update(updateData).eq('id', editingProduct.id);
        if (error) throw error;
        toast({ title: 'Updated', description: 'Product updated successfully' });
      } else {
        const { error } = await supabase.from('products').insert({ name: name.trim(), category_id: categoryId, price: priceNum, description: description.trim(), image_url: imageUrl });
        if (error) throw error;
  ...
  return (
    <div>
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2"><Label>Description *</Label><textarea value={description} onChange={(e) => setDescription(e.target.value)} required rows={3} className="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
        ...
        <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm min-w-[140px]">
          <option value="all">All Categories</option>
          {categories.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
        </select>
        <div className="relative">
          <button onClick={() => setShowSort(!showSort)} className="inline-flex items-center gap-2 h-10 px-4 rounded-md border border-input bg-background text-sm font-medium whitespace-nowrap">{sortLabels[sortBy]}<ChevronDown className={`w-4 h-4 transition-transform ${showSort ? 'rotate-180' : ''}`} /></button>
          {showSort && (
            <div className="absolute right-0 top-12 z-20 bg-card border border-border rounded-lg shadow-lg py-1 min-w-[180px]">
              {(Object.keys(sortLabels) as SortOption[]).map((key) => (
                <button key={key} onClick={() => { setSortBy(key); setShowSort(false); }} className={`block w-full text-left px-4 py-2 text-sm transition-colors ${sortBy === key ? 'bg-primary/10 text-primary font-semibold' : 'text-foreground hover:bg-secondary'}`}>{sortLabels[key]}</button>
              ))}
            </div>
          )}
        </div>
